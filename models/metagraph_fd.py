...
class MetaGraph_fd(nn.Module):
    def __init__(self, hidden_dim, input_dim, sigma=2.0, meta_graph_vertex_num=128):
        ...
        self.meta_graph_vertex = nn.Parameter(torch.rand(meta_graph_vertex_num, input_dim))     # Vk for accumulation
    ...
    def forward(self, inputs):
        correlation_meta = self._correlation(self.meta_graph_vertex_buffer, self.meta_graph_vertex.detach())    # cosine similarity

        self.meta_graph_vertex_buffer = self.meta_graph_vertex.detach()

        batch_size = inputs.size(0)
        protos = inputs
        meta_graph = self._construct_graph(self.meta_graph_vertex, self.meta_graph_vertex).to(self.device)      # (5) MLP+Sigmoid Ak from Vk
        proto_graph = self._construct_graph(protos, protos).to(self.device)  # (4) MLP+Sigmoid As from Vs
        m, n = protos.size(0), self.meta_graph_vertex.size(0)       # m=Nb(batchsize) for Vs =16, n=Nk(batchsize) for Vk =64
        xx = torch.pow(protos, 2).sum(1, keepdim=True).expand(m,n)  # (batchsize,feature_dim)  每一行都表示 protos 中对应样本的平方和
        yy = torch.pow(self.meta_graph_vertex, 2).sum(1, keepdim=True).expand(n, m).t()
        dist = xx + yy  # dist[i][j] 表示样本 protos[i] 和 meta_graph_vertex[j] 之间的平方距离的一部分。
        dist.addmm_(mat1=protos, mat2=self.meta_graph_vertex.t(), beta=1, alpha=-2)     #  每对样本之间的 平方欧几里得距离。
        dist_square = dist.clamp(min=1e-6)
        cross_graph = self.softmax((- dist_square / (2.0 * self.sigma))).to(self.device)        # (6) Ac :cross_graph distance
        super_garph = torch.cat((torch.cat((proto_graph, cross_graph),  dim=1), torch.cat((cross_graph.t(), meta_graph), dim=1)), dim=0)        # Aj
        feature = torch.cat((protos, self.meta_graph_vertex), dim=0).to(self.device)        # (7) Vj
        representation = self.meta_GCN(feature, super_garph)        # (8)

        # most_similar_index = torch.argmax(cross_graph, dim=1)
        correlation_transfer_meta = self._correlation(representation[batch_size:].detach(), self.meta_graph_vertex.detach())    # 经过 GCN 更新后的元图节点表示 与 原始元图节点表示 之间的cosine相似度。
        correlation_protos = self._correlation(representation[0:batch_size].detach(), protos.detach())  # (9) 经过 GCN 更新后的参考样本表示 与 原始参考样本表示 之间的cosine相似度。
    ...
    def _construct_graph(self, A, B):
        ...
        graph = self.distance(diff).view(m, n)      # MLP+Sigmoid (4)
