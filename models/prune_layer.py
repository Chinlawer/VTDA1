...
class SubnetLinear(nn.Linear):
    def __init__(self, in_features, out_features, sparsity=0.5, bias=False, trainable=True):
        ...
        self.w_m = nn.Parameter(torch.empty(out_features, in_features))     # score 随机初始化
    ...
    def forward(self, x, weight_mask=None, bias_mask=None, mode='train', current_sparsity=None):
        w_pruned, b_pruned = None, None
        # 支持 MPR 传进来的动态阈值，如果没有传入则使用默认的 self.sparsity
        sparsity_to_use = current_sparsity if current_sparsity is not None else self.sparsity
